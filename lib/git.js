...
Interface.prototype.status = function(options) {
    var self = this;
    options = options || {};
    ...
    */
                        ...
                        return self.isTagged(options).then(function(isTagged) {
                            if (isTagged) {
...
Interface.prototype.isTagged = function(rev, options) {
    ...
    var self = this;
    return this.callGit(args, options).then(function(result) {
        result = result.replace(/\n$/, "");
        if (result) {
            if (rev) {
...
Interface.prototype.show = function(treeish, file, options) {
    var self = this;
    return self.callGit([
        "show",
        treeish + ":" + file
...
Interface.prototype.callGit = function(procArgs, options) {
    ...
    if (!options.verbose || options.mode === "exec") {
        ...
        EXEC("git " + procArgs.join(" "), {
            ...
            if (error) {
                deferred.reject(new Error("Git error: " + stdout + " " + stderr + " (git " + procArgs.join(" ") + " (cwd: " + (options.cwd || self.path) + ")"));
                return;
            }
            if (/^fatal:/.test(stdout) || /^fatal:/.test(stderr)) {
                deferred.reject(new Error("Git error: " + stdout + " " + stderr));
                return;
            }
            if (options.verbose) {
        ...
        proc.on("exit", function(code) {
            if (code !== 0) {
                deferred.reject(new Error("Git error: " + buffer + " (git " + procArgs.join(" ") + " (cwd: " + (options.cwd || self.path) + ")"));
                return;
            }
