...
Interface.prototype.isRepository = function(callback) {
    return (FS.exists || PATH.exists)(PATH.join(this.path, ".git"), function(exists) {
...
Interface.prototype.clone = function(url, options) {
    ...
    if ((FS.existsSync || PATH.existsSync)(self.path)) {
            ...
            result.split("\n").map(function(remote) {
                ...
                waitfor(function(done) {
                    return self.callGit([
                        ...
                        var m;
                        remotes[remote] = {
                            "fetch-url": ((m = result.match(/Fetch URL: ([^\n]*)\n/)) && m[1]) || false,
                            "push-url": ((m = result.match(/Push  URL: ([^\n]*)\n/)) && m[1]) || false,
                            "branches": {},
                            "remoteBranches": []
                        };
                        var section = null;
...
// Files that have changed and are not comitted.
Interface.prototype.changedFiles = function(callback) {
    var self = this;
    return self.isRepository(function(err, isRepository) {
        if (err) return callback(err);
        if (!isRepository) {
...
//console.log("result", result);
            result.split("\n").forEach(function(line) {
                // TODO: Detect more types of changes. Maybe use external lib?
...
Interface.prototype.canPush = function() {
    return this.push({
        ...
        if (/remote ref does not exist/.test(err.message)) {
            return true;
        }
    ...
    self.isRepository(function(err, isRepository) {
        ...
        if (options.add) {
            if (Array.isArray(options.add)) {
                return self.callGit([
                    "add"
                ].concat(options.add), options, commit);
            } else {
                return self.callGit([
                    "add",
                    "."
